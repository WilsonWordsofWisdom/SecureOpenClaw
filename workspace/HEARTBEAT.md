...
- **Per-tool-call check:** Verify token count and turn count before each tool call. Log a ⚠️ CONTEXT HEALTH warning to logs/security_audit.log if any soft limit is reached.
- **Session close summary:** At the end of each session, log total token usage and turn count to logs/security_audit.log with fields: session_id, total_tokens, total_turns, soft_limit_breaches, hard_limit_breaches.